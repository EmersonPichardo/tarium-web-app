...
function EntradasForm(props) {
    ...
    useEffect(() => {
        if (id) {
            Promise.all([
                ...
                fetch(`https://localhost:44306/api/entradas/${id}`, { method: 'GET' }).then(response => response.json())
            ])
                .then(responses => {
                    setPure(responses[0]);
    ...
    const onFinish = (values) => {
        ...
        if (id) {
            link = `https://localhost:44306/api/entradas/${id}`;
            method = 'PUT';
            values.id = parseInt(id);
        } else {
            link = 'https://localhost:44306/api/entradas';
            method = 'POST';
        }
    ...
    return (
        <PageContent title="Entradas" subtitle="Formulario">
            <Row>
                <Col span={8}>
                    <Title level={5} style={{ marginBottom: '24px' }}>{id ? 'Editar entrada' : 'Agregar entrada'}</Title>
                    ...
                    >
                        ...
                        <Item label="Costo" name="costo"
                            ...
                            hasFeedback>
                            <InputNumber addonBefore="$" min={0.01} step={0.01} style={{ width: '100%' }} />
                        </Item>
